-- =============================================================================
-- HRMS Modules Seed for Config DB - Run in pgAdmin
-- Database: Bnc_Configurator (Config DB)
-- BNC Motors க்கு HRMS project modules insert
-- =============================================================================

-- Step 1 (optional): Quick check - companies, projects
SELECT '1. BNC Company' AS step;
SELECT id, name, code FROM companies WHERE code = 'BNC' OR name ILIKE '%BNC Motors%';

SELECT '2. HRMS Project' AS step;
SELECT p.id, p.name, p.code, p.company_id 
FROM projects p 
WHERE p.code = 'HRMS';

-- Step 2: Seed HRMS modules (WHERE NOT EXISTS = safe to run multiple times)
DO $$
DECLARE
  v_company_id TEXT;
  v_project_id TEXT;
...
BEGIN
  ...
  RAISE NOTICE 'Done. HRMS modules seeded for company % project %', v_company_id, v_project_id;
END $$;
