...
export default function TransferPromotionEntryPage() {
  ...
  const handleExportExcel = () => {
    ...
    const headers = ['Associate Code', 'Associate Name', 'Effective Date'];
    const rows = records.map((r) => [r.associateCode, r.associateName, r.effectiveDate]);
    ...
    link.href = url;
    link.setAttribute('download', `transfer-promotion-entry_${new Date().toISOString().slice(0, 10)}.csv`);
  ...
  const handleExportPdf = () => {
    ...
    const rowsHtml = records.map((r) => `<tr><td>${r.associateCode}</td><td>${r.associateName}</td><td>${r.effectiveDate}</td></tr>`).join('');
    const html = `<!DOCTYPE html><html><head><title>Transfer and Promotion Entry Export</title><style>body{font-family:system-ui,sans-serif;padding:16px}table{width:100%;border-collapse:collapse}th,td{border:1px solid #ddd;padding:8px;font-size:12px}th{background:#f3f4f6;text-align:left}</style></head><body><h2>Transfer and Promotion Entry</h2><table><thead><tr><th>Associate Code</th><th>Associate Name</th><th>Effective Date</th></tr></thead><tbody>${rowsHtml}</tbody></table><script>window.onload=function(){window.print()}<\/script></body></html>`;
        ...
        <div className="w-full max-w-[1600px] mx-auto">
          <div className="flex flex-wrap items-center justify-between gap-4 mb-5">
            <nav className="flex items-center gap-1.5 text-sm text-gray-500" aria-label="Breadcrumb">
              ...
              <span className="text-gray-500">Transfer and Promotion Entry</span>
            </nav>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            <div className="bg-white rounded-lg border border-gray-200 shadow-sm p-4 flex items-center gap-4">
              <div className="flex-shrink-0 w-12 h-12 rounded-full bg-violet-600 flex items-center justify-center">
              ...
              <table className="min-w-full divide-y divide-gray-200 table-fixed">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th className="w-[16%] px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Action</th>
                  </tr>
                </thead>
                <tbody className="bg-white divide-y divide-gray-200">
                  {loading ? (
                    ...
                    <tr>
                      <td colSpan={4} className="px-4 py-8 text-center text-gray-500">No records found. Add one using the Add button.</td>
                    </tr>
                  ) : (
                    sortedRecords.map((record) => (
                      <tr key={record.id} className="hover:bg-gray-50">
                        <td className="w-[28%] px-4 py-4 whitespace-nowrap text-left">
                        ...
                        <td className="w-[28%] px-4 py-4 whitespace-nowrap text-sm text-gray-900 text-left truncate">{record.associateName}</td>
                        <td className="w-[28%] px-4 py-4 whitespace-nowrap text-sm text-gray-600 text-left">{record.effectiveDate}</td>
                        <td className="w-[16%] px-4 py-4 whitespace-nowrap text-right">
                          <button onClick={() => handleView(record)} className="text-blue-600 hover:text-blue-800 p-1 rounded" title="View">
                ...
                {!viewLoading && !viewError && viewDetail && (
                  <div className="space-y-5">
                    <dl className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-3 text-sm">
                      ...
                      <div className="flex gap-3 sm:gap-4">
                        <dt className="text-gray-500 font-medium shrink-0 w-28 sm:w-32">Remarks</dt>
                        <dd className="text-gray-900">{viewDetail.remarks ?? '-'}</dd>
                      </div>
                      <div className="flex gap-3 sm:gap-4">
                      ...
                      {viewDetail.promotionEnabled && (
                        <>
                          <div className="flex gap-3 sm:gap-4">
                            <dt className="text-gray-500 font-medium shrink-0 w-28 sm:w-32">Promotion From</dt>
                            <dd className="text-gray-900">{viewDetail.promotionFrom?.title ?? '-'}</dd>
                          </div>
                          <div className="flex gap-3 sm:gap-4">
                            <dt className="text-gray-500 font-medium shrink-0 w-28 sm:w-32">Promotion To</dt>
                            <dd className="text-gray-900">{viewDetail.promotionTo?.title ?? '-'}</dd>
                          </div>
                        </>
                      )}
                    </dl>
                    {viewDetail.transferComponents && Array.isArray(viewDetail.transferComponents) && viewDetail.transferComponents.length > 0 && (
                      <div className="border-t border-gray-200 pt-4">
                        <h3 className="text-sm font-medium text-gray-700 mb-3">Transfer Components</h3>
                          ...
                          <table className="min-w-full divide-y divide-gray-200">
                            <thead className="bg-gray-50">
                              <tr>
                                ...
                                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wide">Current Value</th>
                                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wide">New Value</th>
                              </tr>
                            </thead>
                            <tbody className="bg-white divide-y divide-gray-200">
                              {viewDetail.transferComponents.map((row: { component: string; currentValue: string; newValue: string }, idx: number) => (
                                <tr key={idx}>
                                  ...
                                  <td className="px-4 py-3 text-sm text-gray-900">{row.currentValue}</td>
                                  <td className="px-4 py-3 text-sm text-gray-900">{row.newValue}</td>
                                </tr>
                              ))}
                            </tbody>
                          </table>
